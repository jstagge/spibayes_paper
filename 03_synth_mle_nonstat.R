...
mle_synth_non_stat_long <- mle_gamma_fit(jdate = synth_non_stat_df$jdate, values = synth_non_stat_df$precip, n = 3000)

### Create WMO 30 year climate normal
short_data <- synth_non_stat_df %>%
	filter(year >= 1961 & year <= 1990)

mle_synth_non_stat_short <- mle_gamma_fit(jdate = short_data$jdate, values = short_data$precip, n = 3000)
...
### Plot the MLE result
mle_plot <- mle_synth_non_stat_long$estimate %>%
...
mle_plot <- mle_synth_non_stat_long$estimate %>%
	mutate(length = "long") %>%
	bind_rows(mle_synth_non_stat_short$estimate %>% mutate(length = "short")) %>%
	pivot_wider(names_from = ci, values_from = value) %>%
	mutate(param = factor(param, levels = c("mean", "scale", "rate", "shape", "disp", "theta"), labels = c("Mean", "Scale", "Rate", "Shape", "Dispersion", "Theta")))
...
#	mutate(line = factor(facet, levels = c("True", "long", "short"), labels = c("True", "Full\n(1920-2019)", "WMO\n(1961-1990)"))) %>%
	mutate(line = factor(facet, levels = c("true", "true_ref", "long", "short"), labels = c("True", "True\n(1961-1990)", "Full\n(1920-2019)", "WMO\n(1961-1990)"))) %>%
...
true_line <- true_plot %>%
	...
	mutate(line = case_when(year >= 1961 & year <= 1990 ~ "true_ref",
		TRUE ~ "true") 
	) %>%
	filter(year %in% seq(1800, 2020, 5))
...
plot_line <- mle_plot %>%
	...
	bind_rows(true_line) %>%
	mutate(facet = factor(facet, levels = c("long", "short"), labels = c("Full (1920-2019)", "WMO (1961-1990)"))) %>%
	mutate(line = factor(line, levels = c("true", "true_ref", "long", "short"), labels = c("True", "True\n(1961-1990)", "Full\n(1920-2019)", "WMO\n(1961-1990)")))
...
p <- ggplot(plot_line %>% filter(param != "Dispersion" & param != "Rate"), aes(x=jdate)) %>%
	...
	+ geom_line(aes(y=value, colour = line, group = year))  %>%
	+ facet_grid(param ~ facet, scales="free_y") %>%
	+ theme_bw(8) %>%
	+ theme(panel.grid.minor = element_blank()) %>%
	+ scale_colour_manual(name = "", values = c("grey50", "black", "#66c2a5", "#fc8d62"), drop = FALSE) %>%
	+ scale_fill_manual(name = "", values = c("grey50", "black", "#66c2a5", "#fc8d62"), drop = FALSE) %>%
	+ scale_x_continuous(name = "Julian Date", breaks=seq(0,365, 30), expand=c(0,0)) %>%
	+ scale_y_continuous(name = "Parameter Value") #%>%
#	+ coord_cartesian(xlim=c(1, 365))
...
### Save Figure
ggsave(file.path(write_figures_path, "mle_fit_non_stat_ref_comparison.png"), p, width =5, height = 7, dpi = 300)
ggsave(file.path(write_figures_path, "mle_fit_non_stat_ref_comparison.pdf"), p, width =5, height = 7)
ggsave(file.path(write_figures_path, "mle_fit_non_stat_ref_comparison.svg"), p, width =5, height = 7)
...
save(mle_synth_non_stat_long, mle_synth_non_stat_short, file = file.path(write_output_path, "mle_synth_non_stat.rda"))
