...
dates_df <- expand.grid(jdate = seq(1,365,1), year = seq(min(precip_j$year),max(precip_j$year),1)) %>%
...
### Loop year by year to prevent memory overflows
	for (year_k in unique(year_seq)){
		...
		dates_k <- seq(as.Date(paste0(year_k, "-01-01")), as.Date(paste0(year_k, "-12-31")), by = "1 day")
		...
		time_index <- which(time_seq %in% dates_k)
